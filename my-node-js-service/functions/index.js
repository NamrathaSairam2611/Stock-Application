...
apiKey.apiKey = "cmr1351r01ql2lmtd0ngcmr1351r01ql2lmtd0o0";
...
app.get("/stock-symbol-lookup", (req, res) => {
  ...
  finnhubClient.symbolSearch(symbol, (error, data) => {
    if (error) {
...
app.get("/news-data", (req, res) => {
  ...
  finnhubClient.companyNews(symbol, fromDate, toDate, (error, data) => {
    if (error) {
      console.error("Error:", error);
      res.status(500).send("Error fetching symbol data");
    } else {
...
app.post("/initialize-wallet", async (req, res) => {
  try {
    ...
    if (existingDocument) {
      await collection.updateOne({}, { $set: { moneyInWallet: 25000 } });
      res.json({ success: true, message: "Wallet initialized successfully" });
    } else {
      await collection.insertOne({ moneyInWallet: 25000 });
      res.json({ success: true, message: "Wallet initialized successfully" });
    }
  } catch (error) {
